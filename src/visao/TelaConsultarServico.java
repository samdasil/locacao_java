...
public class TelaConsultarServico extends javax.swing.JInternalFrame {

    public TelaConsultarServico() {
        initComponents();
        listarServico();
    }
    ...
    private void initComponents() {
        ...
        setBorder(null);
        setClosable(true);
        setTitle("KoT - Serviços");
        setToolTipText("");
        setFont(new java.awt.Font("Abel", 0, 12)); // NOI18N
        setFrameIcon(new javax.swing.ImageIcon(getClass().getResource("/imagens/servico-20x20.png"))); // NOI18N
        ...
        tbServico.setFont(new java.awt.Font("Abel", 0, 14)); // NOI18N
        ...
        lbLogo.setIcon(new javax.swing.ImageIcon(getClass().getResource("/imagens/servico-48x48.png"))); // NOI18N
        lbLogo.setText("Serviços");
    ...
    private void btVisualizarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btVisualizarActionPerformed
        ...
        int celula = tbServico.getSelectedRow();
        
        //VALIDA SELEÇÃO DE LINHA 
        if(celula == -1){
            JOptionPane.showMessageDialog(null, "Selecione um registro !");
        }else{
            TelaVisualizarServico obj = new TelaVisualizarServico(null, true);
                   
            servico.setCodigo((int) tbServico.getModel().getValueAt(celula, 0));
            servico.setDescricao(tbServico.getModel().getValueAt(celula, 1).toString());
            
            obj.receberDados(servico);
            obj.setVisible(true);
    ...
    private void btInserirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btInserirActionPerformed
        TelaCadastroServico obj = new TelaCadastroServico(null, true);
        obj.setVisible(true);
        listarServico();
    ...
    private void btAlterarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btAlterarActionPerformed
        ...
        if(celula == -1){
            ...
            TelaEditarServico obj = new TelaEditarServico(null, true);
    ...
    //METODO PARA PREENCHER A TABELA OFICINA
    public void listarServico(){
        try {
            List<Servico> lista = new ServicoDao().listar();
            DefaultTableModel tabela = (DefaultTableModel) tbServico.getModel();
            
            //tbServico.getColumnModel().getColumn(0).setPreferredWidth(5);
            //tbServico.getColumnModel().getColumn(1).setPreferredWidth(20);            
            tabela.setNumRows(0);
            
            for(Servico servico: lista){
                tabela.addRow(new Object[]{servico.getCodigo(), servico.getDescricao()});
            }//fim for
        } catch (SQLException ex) {
            JOptionPane.showMessageDialog(null, "Erro ao listar: "+ex.getMessage());
        }
    }//fim listagem de servico
    
    //METODO PARA PREENCHER A TABELA MARCA POS BUSCA
    public void buscarServico(){
        try {
            String busca = jtBuscar.getText();
            List<Servico> lista = new ServicoDao().buscar(busca);
            if(lista.isEmpty()){
    ...
    private void deletar(){
        int celula = tbServico.getSelectedRow();
        int codigo = (int) tbServico.getModel().getValueAt(celula, 0);
        try {
            
            int opcaoExc = JOptionPane.showConfirmDialog(null, "Tem certeza que deseja EXCLUIR a servico: "+servico.getDescricao(), "Exclusão" ,JOptionPane.YES_NO_OPTION);
            
            if(opcaoExc == JOptionPane.YES_OPTION){
                servicoDao.deletar(codigo);
